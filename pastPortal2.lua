...
local levelList    = {[stats.LVL_REPR] = {}}
local mov          = stats.movement
local img          = stats.images
local isOpen       = false
local selection    = 1
local currentTab   = stats.LVL_REPR
...
function pastPortal2.open()
    Misc.pause()
    SFXPlay("enter")
    isOpen       = true
    selection    = 1
    currentTab   = stats.LVL_REPR
    movementOver = false
    confActive   = 0
    menuOpacity  = 0
    fadeType     = 1
...
registerEvent(pastPortal2, "onStart")
registerEvent(pastPortal2, "onDraw")
registerEvent(pastPortal2, "onInputUpdate")

function pastPortal2.onStart()
...
function pastPortal2.onDraw()
    ...
    Graphics.drawImageWP(img.titler, 400-img.title.width/2-32, mov.title.position+32, menuOpacity, priorities.tabs)
    draw9Slice{texture = img.frame1r, x = 12, y = mov.list.position,     w = 512, h = 456, priority = priorities.tabs, color = Color.white..menuOpacity}
    draw9Slice{texture = img.frame1r, x = mov.details.position, y = 124, w = 248, h = 456, priority = priorities.tabs, color = Color.white..menuOpacity}
    --draw9Slice{texture = img.frame1r, x = mov.details.position+142, y = 70, w = 106, h = 50, priority = priorities.tabs, color = Color.white..menuOpacity}
    ...
    --[[for i = 1, 1 do -- the 3 tabs
        draw9Slice{texture = img.frame1r, x = mov.tabs.position + ((i-1)*124), y = 60, w = 120, h = 60, priority = priorities.tabs, color = Color.white..menuOpacity}
    ...
    if SaveData.unlockedTabs[currentTab] then
        if currentTab == stats.LVL_REPR then -- details
        ...
        for k, v in ipairs(levelList[currentTab]) do -- level list
            ...
            Graphics.drawImageWP(img.boxSmall, 12 + stats.listOffsetX + 430, ((k-1) * 32) + mov.list.position + stats.listOffsetY - 2, menuOpacity, priorities.stuff)

            if SaveData.levelStats[v.filename] and SaveData.levelStats[v.filename].beaten then
                Graphics.drawImageWP(img.checkSmall, 12 + stats.listOffsetX + 430, ((k-1) * 32) + mov.list.position + stats.listOffsetY - 2, menuOpacity, priorities.stuffHigh)
            end
    ...
    draw9Slice{texture = img.frame1r, x = 400, y = 300, w = 600, h = 120, priority = priorities.confirm, color = Color.white..confOpacity, pivot = Sprite.align.CENTER}
...
function pastPortal2.onInputUpdate()
    if not isOpen then return end
    local totalStars = math.max(SaveData._basegame.starcoinCounter - SaveData.spentStars, 0)
        ...
        elseif player.rawKeys.jump == KEYS_PRESSED then
            ...
            elseif confirmSel == 1 then
                if confActive == stats.CON_START_LVL then
                    ...
                    waitFunc = function()
                        Misc.unpause()
                        GameData.inRepressedMemory = true
