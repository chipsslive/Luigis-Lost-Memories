...
local mpTrigs = {};
local mpLuaTrigs = {};
multipoints.IMG_PATH = getSMBXPath().."\\scripts\\Legacy\\multipoints\\mp.png";
multipoints.IMG_SIZE = { width = 32, height = 32 }

multipoints.__midpointNPCIDs = nil;
...
local function createLuaMT(id)
	...
	mpmt.__index = 
				function(t,k)
					...
					elseif(k == "reset") then
						return function()
							if(multipoints.getCheckpointID() == id) then
								dataFile:set("MPCheck_ID",tostring(-1));
								if(multipoints.midpointCount > 0) then
									mem(0x00B250B0, FIELD_STRING, "")
								end
							end
							mpLuaTrigs[t.id].spawnAt = false;
							dataFile:set("MPCheck_Get"..id,tostring(0));
							dataFile:save()
						end
...
local function warpPlayer(domusic)
...
local function setupNPCS()
	if( multipoints.__midpointNPCIDs ) then
		for _,c in ipairs(NPC.get( multipoints.__midpointNPCIDs)) do
			c = pnpc.wrap(c);
			c.data._basegame.multipoint = multipoints.addLuaCheckpoint(c.x + c.width*0.5 + multipoints.__midpointNPCSpawnOffsets[c.id].x, c.y + c.height*0.5 + multipoints.__midpointNPCSpawnOffsets[c.id].y, c:mem(0x146, FIELD_WORD));
			c.data._basegame.multipoint.visible = false;
		end
	end
end
