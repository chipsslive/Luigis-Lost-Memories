...
npcManager.setNpcSettings(strongGraySettings)
...
--Register events
function strongGray.onInitAPI()
	--npcManager.registerEvent(npcID, strongGray, "onTickNPC")
	npcManager.registerEvent(npcID, strongGray, "onTickEndNPC")
	--npcManager.registerEvent(npcID, strongGray, "onDrawNPC")
	registerEvent(strongGray, "onNPCKill")
end

function strongGray.onTickEndNPC(v)
	...
	v.animationFrame = 4 - data.strength
...
function strongGray.onNPCKill(eventObj, v, killReason, culprit)
	...
	if data.strength ~= 0 then
		...
		SFX.play(4)
		Animation.spawn(100,v.x+32,v.y+32)
	end
	data.respite = true
end
